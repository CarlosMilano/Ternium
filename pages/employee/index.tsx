...
import EditSection, {
  EditEventHandler,
  Editable,
} from "@/components/employee/EditSection";
import {
  ChangeEvent,
  ChangeEventHandler,
  FormEvent,
...
const EmployeePage: React.FC = (): JSX.Element => {
  ...
  const [fetchedData, updateFetchedData, setFetchedData] =
    useObjectState<AllData>({
      dataEmpleado: null,
  ...
  const [dataEmpleado, updateDataEmpleado, setDataEmpleado] =
    useObjectState<TableEmpleado | null>(null);
  const [dataComentarios, updateDataComentarios, setDataComentarios] =
    useObjectState<TableComentarios[] | null>(null);
  const [dataResumen, updateDataResumen, setDataResumen] = useObjectState<
    TableResumen[] | null
  >(null);
  const [dataEvaluacion, updateDataEvaluacion, setDataEvaluacion] =
    useObjectState<TableEvaluacion[] | null>(null);
  const [dataTrayectoria, updateDataTrayectoria, setDataTrayectoria] =
    useObjectState<TableTrayectoria[] | null>(null);
  ...
  const [isDisableDialogOpened, setIsDisableDialogOpened] =
  ...
  const handleOnEdit: EditEventHandler = (_, index) =>
  ...
  const handleOnSubmitEmpleado: FormEventHandler<HTMLFormElement> = async (
  ...
  const handleOnSubmitComentarios: FormEventHandler<HTMLFormElement> = async (
    e: FormEvent<HTMLFormElement>
  ) => {
    e.preventDefault();
    setIsUpdatingData(true);
    dataComentarios?.forEach(
      async (comentario: TableComentarios, index: number) => {
        try {
          ...
          if (res.ok) {
            ...
            resetProperty(
              updateFetchedData,
              `dataComentarios.${index}`,
              comentario
            );
            // updateFetchedData(`dataComentarios.${index}`, comentario);
          } else {
            console.error(
              "Error updating data for Comentarios: ",
              res.statusText
            );
            resetProperty(
              updateDataComentarios,
              `${index}`,
              fetchedData.dataComentarios?.at(index)
            );
  ...
  const handleOnSubmitResumen: FormEventHandler<HTMLFormElement> = async (
    e: FormEvent<HTMLFormElement>
  ) => {
    e.preventDefault();
    setIsUpdatingData(true);
      ...
      try {
        ...
        if (res.ok) {
          ...
          resetProperty(
            updateDataResumen,
            `${index}`,
            fetchedData.dataResumen?.at(index)
          );
  ...
  const handleOnSubmitEvaluacion: FormEventHandler<HTMLFormElement> = async (
    e: FormEvent<HTMLFormElement>
  ) => {
    e.preventDefault();
    setIsUpdatingData(true);
    dataEvaluacion?.forEach(
      async (evaluacion: TableEvaluacion, index: number) => {
        try {
          ...
          if (res.ok) {
            ...
            resetProperty(
              updateFetchedData,
              `dataEvaluacion.${index}`,
              evaluacion
            );
            // updateFetchedData(`dataEvaluacion.${index}`, evaluacion);
          } else {
            console.error(
              "Error updating data for Evaluacion: ",
              res.statusText
            );
            resetProperty(
              updateDataEvaluacion,
              `${index}`,
              fetchedData.dataEvaluacion?.at(index)
            );
  ...
  const handleOnSubmitTrayectoria: FormEventHandler<HTMLFormElement> = async (
    e: FormEvent<HTMLFormElement>
  ) => {
    e.preventDefault();
    setIsUpdatingData(true);
    dataTrayectoria?.forEach(
      async (trayectoria: TableTrayectoria, index: number) => {
        try {
          ...
          if (res.ok) {
            ...
            resetProperty(
              updateFetchedData,
              `dataTrayectoria.${index}`,
              trayectoria
            );
          } else {
            console.error(
              "Error updating data for Trayectoria: ",
              res.statusText
            );
            resetProperty(
              updateDataTrayectoria,
              `${index}`,
              fetchedData.dataTrayectoria?.at(index)
            );
          }
        } catch (err) {
  ...
  const handleOnClickDeleteComentario: (id_comentario: number) => void = async (
    id_comentario
  ) => {
    ...
    try {
      ...
      const updatedComments: TableComentarios[] = dataComentarios.filter(
        (comentario: TableComentarios) => {
        ...
        <Container>
          <Grid container gap={1} justifyContent='center' paddingBottom={8}>
            ...
            <Grid item xs={12} md={8}>
              <Paper
                variant='outlined'
                sx={{ padding: "1.5rem", paddingTop: "1rem", height: "100%" }}
              >
                {/* Top section */}
                <EditSection
                  index={0}
                    ...
                    {dataEvaluacion ? (
                      dataEvaluacion?.map(
                        (
                          ...
                          const onChangePerformance: ChangeEventHandler<
                            HTMLInputElement
                          > = (e: ChangeEvent<HTMLInputElement>) => {
                            updateDataEvaluacion(
                              `${index}.performance`,
                          ...
                          const onChangePotencial: ChangeEventHandler<
                            HTMLInputElement
                          > = (e: ChangeEvent<HTMLInputElement>) => {
                            updateDataEvaluacion(
                              `${index}.potencial`,
                          ...
                          const onChangeCurva: ChangeEventHandler<
                            HTMLInputElement
                          > = (e: ChangeEvent<HTMLInputElement>) => {
                            updateDataEvaluacion(
                              `${index}.curva`,
                    ...
                    {dataTrayectoria ? (
                      dataTrayectoria?.map(
                        (
                          ...
                          const onChangeEmpresa: ChangeEventHandler<
                            HTMLInputElement
                          > = (e: ChangeEvent<HTMLInputElement>) => {
                            updateDataTrayectoria(
                              `${index}.empresa`,
                          ...
                          const onChangePuesto: ChangeEventHandler<
                            HTMLInputElement
                          > = (e: ChangeEvent<HTMLInputElement>) => {
                            updateDataTrayectoria(
                              `${index}.puesto`,
